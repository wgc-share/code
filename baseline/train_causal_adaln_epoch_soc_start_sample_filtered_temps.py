...
PROJECT = "PINT_SchemeB_CAUSAL_ADALN_EPOCH_SOC_START_SAMPLE_FILTERED_TEMPS"
...
class EpochSOCStartSampledDataset:
    def __init__(self, base_dataset: BatteryTDGCMDataset, starts: list[float], seed: int):
        self.base_dataset = base_dataset
        self.samples, self.assignment_rows = self._build_samples(base_dataset, starts, seed)
    ...
    @staticmethod
    def _build_samples(base_dataset: BatteryTDGCMDataset, starts: list[float], seed: int):
        ...
        for filename in sorted(file_to_samples):
            ...
            start_soc = float(rng.choice(starts))
            ...
            start_label = f"{start_soc:.4f}".replace(".", "p")
            subsegment_name = f"{filename}__epoch_socstart_{start_label}"
    ...
    @classmethod
    def from_base(cls, base_dataset: BatteryTDGCMDataset, starts: list[float], seed: int):
        obj = cls.__new__(cls)
        obj.base_dataset = base_dataset
        obj.samples, obj.assignment_rows = cls._build_samples(base_dataset, starts, seed)
        return obj


def _make_results_dirs():
    base = baseline_results_dir() / "epoch_soc_start_sample_filtered_temps"
    cache_dir = base / "pt"
...
def print_config(config: dict):
    print("=== SchemeB causal + AdaLN + epoch SOC-start sampled run ===")
    ...
    print(f"TRAIN_STARTS: {config['TRAIN_SOC_STARTS']}")
...
def load_all_datasets(config: dict):
    ...
    train_ds = BatteryTDGCMDataset(
        ...
        cache_file=os.path.join(config["CACHE_DIR"], f"train_cache_causal_adaln_epoch_soc_start_sample_{suffix}.pt"),
    )
    val_ds = BatteryTDGCMDataset(
        config["DATA_DIR"], val_f, window_size=config["WINDOW_SIZE"], stride=config["STRIDE"],
        cache_file=os.path.join(config["CACHE_DIR"], f"val_cache_causal_adaln_epoch_soc_start_sample_{suffix}.pt"),
    )
    test_random_ds = BatteryTDGCMDataset(
        config["DATA_DIR"], test_random_f, window_size=config["WINDOW_SIZE"], stride=config["STRIDE"],
        cache_file=os.path.join(config["CACHE_DIR"], f"test_random_cache_causal_adaln_epoch_soc_start_sample_{suffix}.pt"),
    )
    test_fixed_ds = BatteryTDGCMDataset(
        config["DATA_DIR"], test_fixed_f, window_size=config["WINDOW_SIZE"], stride=config["STRIDE"],
        cache_file=os.path.join(config["CACHE_DIR"], f"test_fixed_cache_causal_adaln_epoch_soc_start_sample_{suffix}.pt"),
    )
...
def train_epoch_soc_start_sample():
    set_seed(42)
    config = make_config()
    ...
    for epoch in range(config["EPOCHS"]):
        model.train()
        train_ds = EpochSOCStartSampledDataset.from_base(
            train_base_ds,
            config["TRAIN_SOC_STARTS"],
            seed=42 + epoch,
        )
        pd.DataFrame(train_ds.assignment_rows).assign(epoch=epoch + 1).to_csv(
            os.path.join(config["META_DIR"], f"epoch_soc_start_sample_assignment_{run_id}_epoch{epoch + 1:03d}.csv"),
            index=False,
            encoding="utf-8-sig",
        )
        ...
        print(
            ...
            f"base_train_windows={len(train_base_ds)} | train_windows={len(train_ds)} | "
            f"subsegments={len(train_ds.assignment_rows)} | val_windows={len(val_ds)} | "
            f"steps={manager.total_steps} | p_reset=disabled"
        )
...
def eval_latest_soc_interval_checkpoint(intervals: list[tuple[float, float]], soc_batch_size: int):
    ...
    print("=== SOC-interval evaluation for latest filtered-temperature epoch SOC-start sampled checkpoint ===")
    print(f"CHECKPOINT : {ckpt_path}")
    print_config(config)
...
def main():
    parser = argparse.ArgumentParser(description="Train or evaluate filtered-temperature causal AdaLN epoch SOC-start sampled baseline.")
    parser.add_argument(
        "--mode",
    ...
    if mode is None:
        ...
        print("1 - start a new epoch SOC-start sampled training run")
