...
RESULT_ROOT = ROOT / "results" / "ablation" / "batch_temp_extrap_10x_logs"

METHODS = [
    ("no_ahloss", "train_ablation_temp_extrap_no_ahloss.py"),
...
def main():
    parser = argparse.ArgumentParser(description="Run the temperature-extrapolation ablation set for 10 rounds.")
    ...
    run_name = args.run_name or f"temp_extrap_10x_{datetime.now().strftime('%m%d_%H%M%S')}"
    ...
    plan_path = run_root / "temp_extrap_10x_plan.csv"
    status_path = run_root / "temp_extrap_10x_status.csv"
    write_csv(plan_path, plan_rows)

    print("=== Temperature extrapolation 10x batch plan ===")
    ...
    print("\nAll scheduled temperature-extrapolation runs finished.")
