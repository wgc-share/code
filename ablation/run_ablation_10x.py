...
RESULT_ROOT = ROOT / "results" / "ablation" / "batch_10x_logs"

METHODS = [
    ("no_adaln_temp_input", "train_ablation_no_adaln_temp_input.py"),
    ("temp_input_adaln_no_ah", "train_ablation_temp_input_adaln_no_ah.py"),
    ("film", "train_ablation_film.py"),
    ("no_state_transfer", "train_ablation_no_state_transfer.py"),
    ("preset0", "train_ablation_preset0.py"),
    ("no_ahloss", "train_ablation_no_ahloss.py"),
]


def write_plan(plan_path: Path, rows: list[dict]):
...
def append_status(status_path: Path, row: dict):
    exists = status_path.exists()
    with status_path.open("a", newline="", encoding="utf-8-sig") as f:
...
def main():
    parser = argparse.ArgumentParser(
        description="Run five filtered-temperature ablation experiments for multiple paired rounds."
    )
    ...
    run_name = args.run_name or f"ablation_5methods_{args.rounds}x_{datetime.now().strftime('%m%d_%H%M%S')}"
    ...
    plan_path = run_root / "ablation_10x_plan.csv"
    status_path = run_root / "ablation_10x_status.csv"
    write_plan(plan_path, plan_rows)

    print("=== Ablation 10x batch plan ===")
    print(f"RUN_ROOT : {run_root}")
    print(f"PLAN     : {plan_path}")
    print(f"STATUS   : {status_path}")
    print(f"ROUNDS   : {args.rounds}")
    print(f"METHODS  : {', '.join(method for method, _ in METHODS)}")
    ...
    for row in plan_rows:
        ...
        return_code = stream_process(cmd, log_path)
    ...
    print("\nAll scheduled ablation runs finished.")
