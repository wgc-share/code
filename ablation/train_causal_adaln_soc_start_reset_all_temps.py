...
PROJECT = "PINT_SchemeB_CAUSAL_ADALN_SOC_START_RESET_ALL_TEMPS"
...
def _make_results_dirs():
    base = organized_results_dir() / "ablation" / "soc_start_reset_all_temps"
    cache_dir = base / "pt"
...
def assign_reset_starts(train_ds: BatteryTDGCMDataset, starts: list[float], seed: int):
    rng = np.random.default_rng(seed)
    filenames = sorted({sample["filenames"] for sample in train_ds.samples})
    return {filename: float(rng.choice(starts)) for filename in filenames}
    ...
    for split_order, (split_name, base_dataset) in enumerate((("test_random", test_random_ds), ("test_fixed", test_fixed_ds)), start=1):
        for soc_order, soc_start in enumerate(config["SOC_STARTS"], start=1):
                ...
                rows.append(
                    {
                        ...
                        "trial_index": 1,
                        "p_reset": math.nan,
                        "reset_strategy": "soc_start_reset",
            ...
            for row in segment_rows:
                row["reset_strategy"] = "soc_start_reset"
...
def make_base_config(args: argparse.Namespace, create_dirs: bool = True):
    ...
    reset_starts = parse_float_list(args.reset_starts)
...
def load_all_datasets(config: dict):
    ...
    print(
        "Temperature scope: all temperatures | "
        f"train={len(train_f)} | "
        f"val={len(val_f)} | "
        f"test_random={len(test_random_f)} | "
        f"test_fixed={len(test_fixed_f)}"
    )
    empty_splits = [
        name
        for name, files in {
    ...
    train_ds = BatteryTDGCMDataset(
        ...
        cache_file=os.path.join(config["CACHE_DIR"], f"train_cache_causal_adaln_soc_start_reset_{suffix}.pt"),
    )
    val_ds = BatteryTDGCMDataset(
        ...
        window_size=config["WINDOW_SIZE"],
        stride=config["STRIDE"],
        cache_file=os.path.join(config["CACHE_DIR"], f"val_cache_causal_adaln_soc_start_reset_{suffix}.pt"),
    )
    test_random_ds = BatteryTDGCMDataset(
        ...
        window_size=config["WINDOW_SIZE"],
        stride=config["STRIDE"],
        cache_file=os.path.join(config["CACHE_DIR"], f"test_random_cache_causal_adaln_soc_start_reset_{suffix}.pt"),
    )
    test_fixed_ds = BatteryTDGCMDataset(
        ...
        window_size=config["WINDOW_SIZE"],
        stride=config["STRIDE"],
        cache_file=os.path.join(config["CACHE_DIR"], f"test_fixed_cache_causal_adaln_soc_start_reset_{suffix}.pt"),
    )
...
def train_soc_start_reset(args: argparse.Namespace):
    ...
    print(f"RESET_STARTS : {config['RESET_STARTS']}")
    print(f"SOC_STARTS   : {config['SOC_STARTS']}")
    print(f"TEMP_SCOPE   : {config['TEMPERATURE_SCOPE']}")
    ...
    reset_start_by_file = assign_reset_starts(train_ds, config["RESET_STARTS"], args.seed)
    pd.DataFrame(
        [{"filename": filename, "reset_start_soc_percent": start} for filename, start in sorted(reset_start_by_file.items())]
    ).to_csv(os.path.join(config["META_DIR"], f"soc_start_reset_assignment_{run_id}.csv"), index=False, encoding="utf-8-sig")
    ...
    for epoch in range(config["EPOCHS"]):
        ...
        print(
            ...
            f"soc_start_reset_lanes={epoch_reset_lanes} | "
            f"soc_start_reset_batches={epoch_reset_batches}"
        )
...
def eval_latest_checkpoint(args: argparse.Namespace):
    ...
    config["TEMPERATURE_SCOPE"] = ckpt_config.get("TEMPERATURE_SCOPE", "all_temps")
...
def eval_latest_soc_interval_checkpoint(args: argparse.Namespace, intervals: list[tuple[float, float]]):
    ...
    print("=== SOC-interval evaluation for latest all-temperature SOC-start-reset checkpoint ===")
    print(f"CHECKPOINT : {ckpt_path}")
    print(f"DATA_DIR   : {config['DATA_DIR']}")
    print(f"SPLIT_FILE : {config['SPLIT_FILE']}")
    print(f"CSV_DIR    : {config['CSV_DIR']}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Train causal AdaLN model with one SOC-start hidden-state reset per segment using all temperatures."
    )
    ...
    parser.add_argument("--reset-starts", default="100,90,80,70,60,50,40,30,20,10")
    ...
    if args.dry_run:
        print("SOC-start reset dry run")
        print(f"reset starts: {parse_float_list(args.reset_starts)}")
        print(f"test SOC starts: {parse_float_list(args.starts)}")
        print("temperature scope: all_temps")
