...
RESULT_ROOT = ROOT / "results" / "ablation" / "batch_245_no_ah_logs"
...
def main():
    parser = argparse.ArgumentParser(description="Run the no-Ah baseline ablations for items 2, 4, and 5.")
    ...
    run_name = args.run_name or f"ablation_245_no_ah_{args.rounds}x_{datetime.now().strftime('%m%d_%H%M%S')}"
    ...
    plan_path = run_root / "ablation_245_no_ah_plan.csv"
    status_path = run_root / "ablation_245_no_ah_status.csv"
    write_csv(plan_path, plan_rows)

    print("=== Ablation 2/4/5 no-Ah batch plan ===")
