...
METHODS = [
    ("no_ahloss", "train_ablation_temp_extreme_no_ahloss.py"),
    ("no_adaln_temp_input_no_ah", "train_ablation_temp_extreme_no_adaln_temp_input_no_ah.py"),
    ("temp_input_adaln_no_ah", "train_ablation_temp_extreme_temp_input_adaln_no_ah.py"),
]
...
def main():
    parser = argparse.ArgumentParser(description="Run three temperature-extreme ablation experiments for multiple rounds.")
    ...
    run_name = args.run_name or f"temp_extreme_3methods_{args.rounds}x_{datetime.now().strftime('%m%d_%H%M%S')}"
    ...
    plan_path = run_root / "temp_extreme_3methods_plan.csv"
    status_path = run_root / "temp_extreme_3methods_status.csv"
    write_csv(plan_path, plan_rows)

    print("=== Temperature-extreme 3-method batch plan ===")
    ...
    print("\nAll scheduled temperature-extreme runs finished.")
